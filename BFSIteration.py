...
class MRBFSItreration(MRJob):
    INPUT_PROTOCOL=RawValueProtocol
    OUTPUT_PROTOCOL=RawValueProtocol
    
    def configure_options(self):
        super(MRBFSItreration,self).configure_options()
        self.add_passthrough_option('--target',help="ID of charcter we are searching for")
        ...
        if(node.color=='GRAY'):
            for connection in node.connections:
                ...
                if(self.options.target==connection):
                    counterName=("Taret ID"+connection+
